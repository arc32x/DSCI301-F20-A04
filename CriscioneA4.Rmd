...
<span style="color: green; font-size: 14pt">*Comments:*  
        * Everything in here looks pretty good, except I think you are choosing your binwidths a bit too small.  (See below.)
    </span>
...
```
  ...
  <span style="color: green; font-size: 14pt">*Comments:*  
  * The spikes on this suggest to me that you might want to think about using a larger binwidth. This might be a bit too granular.
  I played around with a few values and I think I might like about 0.3. </span>
  
```{r}
ggplot(iris, aes(Sepal.Width)) +
  geom_histogram(binwidth = 0.3) +
  labs(title = "Sepal Width Distribution: Rebinned", x = "Sepal Width", y = "Count")
```
...
<span style="color: green; font-size: 14pt">*Comments:*  
        * I really like the use of the frequency polygon here, but it still looks 
        too granular.  (Notice how busy the polygons look.)
        I think .35 helps separate these a bit: 
    </span>
```{r}
ggplot(iris, aes(x = Sepal.Width, color = Species)) +
  geom_freqpoly(binwidth = 0.35) +
  labs(title = "Sepal Width Distribution by Species",
       x = "Sepal Width", y = "Count")
```
...
<span style="color: green; font-size: 14pt">*Comments:*  
        * All fair observations.  Looks good to me!
    </span>
